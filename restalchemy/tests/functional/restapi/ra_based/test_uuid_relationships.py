...
class UuidRelPortRoute(routes.Route):
    ...
    __allow_methods__ = [routes.CREATE, routes.FILTER, routes.GET]  # noqa: RUF012
...
class UuidRelRuleRoute(routes.Route):
    ...
    __allow_methods__ = [routes.CREATE, routes.FILTER, routes.GET, routes.UPDATE]  # noqa: RUF012
...
class UuidRelationshipsTestCase(base.BaseWithDbMigrationsTestCase):
    ...
    def _url(self, path):
        return f"http://127.0.0.1:{self._port_num}{path}"

    @staticmethod
    def _vm_uri(vm):
        return f"/vms/{vm.uuid}"

    @staticmethod
    def _port_uri(port):
        return f"/vms/{port.vm.uuid}/ports/{port.uuid}"

    def _post_rule(self, collection="rules", **body):
        body.setdefault("vm", str(VM1))
        body.setdefault("port", self._port_uri(self._port))
        return requests.post(self._url(f"/{collection}/"), json=body)
    ...
    def test_a_bare_uuid_names_the_related_resource(self):
        ...
        self.assertEqual(
            self._url("/rules/{}".format(body["uuid"])),
    ...
    def test_the_field_is_still_read_back_as_a_uri(self):
        ...
        response = requests.get(self._url(f"/rules/{rule_uuid}"))
    ...
    def test_a_bare_uuid_filters_the_collection(self):
        ...
        response = requests.get(self._url(f"/rules/?vm={VM1}"))
