...
Связь, которую запрос не прочитал вместе со строкой (`prefetch`), приходит
идентификатором, и модель, которую он называет, читается отдельно. `get_all()`
и `query()` собирают идентификаторы всей страницы и читают их одним запросом на
связь, а не одним запросом на строку, и так же поступают с тем, на что
указывают уже прочитанные объекты.

- Строки страницы, называющие один и тот же объект, получают один и тот же
  экземпляр.
- Идентификатор, которого страница не нашла, остаётся как есть, поэтому строка,
  указывающая на отсутствующую запись, падает ровно так же, как падала раньше.
- Связь, объявленная с `prefetch=True`, читается тем же запросом, что и
  страница (`LEFT JOIN`), и в этом не участвует.
- `SQLStorableMixin.RELATIONSHIP_BATCH_SIZE` (по умолчанию 1000) ограничивает
  количество идентификаторов в одном запросе.
