...
сервера и TCP. Продолжительность задаётся в секундах. Если опция не указана,
сохраняется соответствующая настройка libpq, PostgreSQL или операционной
системы. Явное значение `0` передаётся драйверу; для серверных таймаутов
PostgreSQL оно отключает таймаут.
