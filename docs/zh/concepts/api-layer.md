...
时间戳按 RFC 3339 以 UTC 写出：`2026-08-16T12:34:56.123456Z`。正好落在整秒上的
时间戳不再带小数部分——`2026-08-16T12:34:56Z`，此前是 `.000000`。嵌套在值内部
（`Dict`、`TypedList`）的时间戳同样以 `Z` 结尾，此前是 `+00:00`。已弃用的朴素
`UTCDateTime` 仍按原样写出。
