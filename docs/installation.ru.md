...
Если вы планируете использовать SQL-хранилище, установите restalchemy с соответствующим драйвером:

```bash
# С поддержкой MySQL
pip install restalchemy[mysql]

# С поддержкой PostgreSQL
pip install restalchemy[pgsql]

# С обеими СУБД
pip install restalchemy[all]
```
